...
productsDF = spark.read.csv(productsCsvPath, header=True, inferSchema=True)
...
productsDF2 = spark.read.csv(productsCsvPath, schema=userDefinedSchema, header=True)
...
productsDF3 = spark.read.csv(productsCsvPath, schema=DDLSchema, header=True)
...
productsDF.write.format("delta").save(path=productsOutputPath)
