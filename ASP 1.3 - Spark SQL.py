...
# TODO
numRows = macDF.count()
rows = macDF.where("event_previous_timestamp IS NOT NULL").limit(5).orderBy("event_previous_timestamp").collect()
...
# TODO
macSQLDF = spark.table("events").where("device = 'macOS'").orderBy("event_timestamp")
